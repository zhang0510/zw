(function($){
	$.fn.Huploadify = function(opts){
		var itemTemp = '<ul id="upload_li"><li><input type="hidden" name="pic_name[]" value=""><div id="${fileID}" class="uploadify-queue-item"><div class="img_height" id="${imags}"></div> <div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><a href="javascript:void(0);" class="uploadbtn">上传</a></div></li></ul>';//<a href="javascript:void(0);" class="delfilebtn">删除</a>
		var defaults = {
			fileTypeExts:'*.jpg;*.gif;*.png;*.jpeg;*.mp4',//允许上传的文件类型，格式'*.jpg;*.doc'
			uploader:'',//文件提交的地址
			auto:false,//是否开启自动上传
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				_renderFile : function(file){
					var $html = $(option.itemTemplate.replace(/\${imags}/g,'fileupload_'+file.index).replace(/\${fileID}/g,'fileupload_'+instanceNumber+'_'+file.index).replace(/\${fileName}/g,file.name).replace(/\${fileSize}/g,F.formatFileSize(file.size)).replace(/\${instanceID}/g,_this.attr('id')));
					//如果是非自动上传，显示上传按钮
					if(!option.auto){
					...
					if(option.showUploadedSize){
						var num = '<p class="progressnum"><span class="uploadedsize">0KB</span>/<span class="totalsize">${fileSize}</span></p>'.replace(/\${fileSize}/g,F.formatFileSize(file.size));
