...
class Database{
    ...
    private function write($sql){
        ...
        print_log("是否启用压缩：".$this->config['compress']);
        $this->open($size); 
    ...
    public function backup($table, $start){
        ...
        print_log("数据库表{$table}信息总查（询起始数{$start}）条数：".json_encode($result));
        $count  = $result['0']['count'];
        print_log("数据库表{$table}信息（询起始数{$start}）(总条数)：".$count);
        //备份表数据
        if($count){
            //写入数据注释
            /*
            ...
            */
            ...
            print_log("数据库表{$table}信（询起始数{$start}）息：".json_encode($result));
            foreach ($result as $row) {
                $row = array_map('mysql_real_escape_string', $row);
                print_log("数据库表{$table}信 单行 息：".json_encode($row));
                $sql = "INSERT INTO `".$table."` VALUES ('" . implode("', '", $row) . "')\n";
                $sql .= ";--www.lfmnet.com--\n";
                print_log("数据库表{$table}信息（询起始数{$start}）（备份）信息：".$sql);
                $flags = $this->write($sql);
                print_log("数据库表{$table}信息（询起始数{$start}）是否备份成功：".$flags);
                if(false === $flags){
                    return false;
                }
            }
